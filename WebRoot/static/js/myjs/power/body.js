	var year = $("#year").val();
	var month = $("#month").val();
	var powerScoreArray = JSON.parse($("#powerScoreArray").val());
	var grid_data = JSON.parse($("#powerIndexListForGrid").val());
		    ...
		    fusioncharts.render();
 			...
 			function query(){
				var year = $("#datepicker").val().split("-")[0];
	 			...
	 			$.ajax({
	 	               type: "GET",
	 	               url: "power/getGridData.do?year="+year+"&&month="+month,
	 			...
	 			$.ajax({
	 	               type: "GET",
	 	               url: "power/getChartData.do?year="+year+"&&month="+month+"&&json="+encodeURIComponent(JSON.stringify(json)),
	 	               success: function(data){
	 	            	   		json=data;
