	var year = $("#year").val();
	var month = $("#month").val();
	var trainScoreArray = JSON.parse($("#trainScoreArray").val());
	var grid_data = JSON.parse($("#trainScoreForGridList").val());
	var fusioncharts = null;

			jQuery(function($) {
				
				$("#lm17").addClass("active");  //设置该页菜单为选中状态
		    ...
		    fusioncharts.render();
 			...
 			function query(){
				var year = $("#datepicker").val().split("-")[0];
	 			...
	 			$.ajax({
	 	               type: "GET",
	 	               url: "trainscore/getGridData.do?year="+year+"&&month="+month,
	 			...
	 			$.ajax({
	 	               type: "GET",
	 	               url: "trainscore/getChartData.do?year="+year+"&&month="+month+"&&json="+encodeURIComponent(JSON.stringify(json)),
	 	               success: function(data){
	 	            	   		json=data;
