	var year = $("#year").val();
	var month = $("#month").val();
	var gasTempArray = JSON.parse($("#gasTempArray").val());
	var grid_data = JSON.parse($("#gasTempListForGridList").val());
	var fusioncharts = null;
			...
			jQuery(function($) {
				...
				$("#lm5 ul").show();
				$("#z8").addClass("active");  //设置该页菜单为选中状态
		    ...
		    fusioncharts.render();
 			...
 			function query(){
				var year = $("#datepicker").val().split("-")[0];
	 			...
	 			$.ajax({
	 	               type: "GET",
	 	               url: "gastemp/getGridData.do?year="+year+"&&month="+month,
	 			...
	 			$.ajax({
	 	               type: "GET",
	 	               url: "gastemp/getChartData.do?year="+year+"&&month="+month+"&&json="+encodeURIComponent(JSON.stringify(json)),
	 	               success: function(data){
	 	            	   		json=data;
