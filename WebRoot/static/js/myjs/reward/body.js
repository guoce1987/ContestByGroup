	var year = $("#year").val();
	var month = $("#month").val();
	var rewardArray = JSON.parse($("#rewardArray").val());
	var grid_data = JSON.parse($("#rewardForGridList").val());
	var fusioncharts = null;
			...
			jQuery(function($) {
				...
				$("#lm5 ul").show();
				$("#z12").addClass("active");  //设置该页菜单为选中状态
		    ...
		    fusioncharts.render();
 			...
 			function query(){
				var year = $("#datepicker").val().split("-")[0];
	 			...
	 			$.ajax({
	 	               type: "GET",
	 	               url: "reward/getGridData.do?year="+year+"&&month="+month,
	 			...
	 			$.ajax({
	 	               type: "GET",
	 	               url: "reward/getChartData.do?year="+year+"&&month="+month+"&&json="+encodeURIComponent(JSON.stringify(json)),
	 	               success: function(data){
	 	            	   		json=data;
