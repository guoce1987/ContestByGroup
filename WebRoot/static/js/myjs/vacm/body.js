	var year = $("#year").val();
	var month = $("#month").val();
	var vacmIndexArray = JSON.parse($("#vacmIndexArray").val());
	var grid_data = JSON.parse($("#vacmIndexForGridList").val());
	var fusioncharts = null;
			...
			jQuery(function($) {
				...
				$("#lm5 ul").show();
				$("#z9").addClass("active");  //设置该页菜单为选中状态
		    ...
		    fusioncharts.render();
 			...
 			function query(){
				var year = $("#datepicker").val().split("-")[0];
	 			...
	 			$.ajax({
	 	               type: "GET",
	 	               url: "vacm/getGridData.do?year="+year+"&&month="+month,
	 			...
	 			$.ajax({
	 	               type: "GET",
	 	               url: "vacm/getChartData.do?year="+year+"&&month="+month+"&&json="+encodeURIComponent(JSON.stringify(json)),
	 	               success: function(data){
	 	            	   		json=data;
