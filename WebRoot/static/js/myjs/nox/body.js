	var year = $("#year").val();
	var month = $("#month").val();
	var noxIndexArray = JSON.parse($("#noxIndexArray").val());
	var grid_data = JSON.parse($("#noxIndexForGridList").val());
	var fusioncharts = null;
			...
			jQuery(function($) {
				...
				$("#lm5 ul").show();
				$("#z10").addClass("active");  //设置该页菜单为选中状态
		    ...
		    fusioncharts.render();
 			...
 			function query(){
				var year = $("#datepicker").val().split("-")[0];
	 			...
	 			$.ajax({
	 	               type: "GET",
	 	               url: "nox/getGridData.do?year="+year+"&&month="+month,
	 			...
	 			$.ajax({
	 	               type: "GET",
	 	               url: "nox/getChartData.do?year="+year+"&&month="+month+"&&json="+encodeURIComponent(JSON.stringify(json)),
	 	               success: function(data){
	 	            	   		json=data;
